...
if [ -z $JAVA_OPTS ]; then
  ...
  exec java -XX:ExitOnOutOfMemoryError -Djava.security.egd=file:/dev/./urandom -jar "$@"
else
  echo "running java with JAVA_OPTS as:" exec java "$JAVA_OPTS" -Djava.security.egd=file:/dev/./urandom -jar "$@"
  exec java -XX:ExitOnOutOfMemoryError "$JAVA_OPTS" -Djava.security.egd=file:/dev/./urandom -jar "$@"
fi
